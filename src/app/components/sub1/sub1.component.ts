...
export class Sub1Component implements OnInit, AfterViewInit {
...
// tslint:disable-next-line:typedef
assign(depart) {
  this.fileService.assignFileSub1(this.assignId, this.assignedfile, this.departUser, depart).subscribe(
    data => {
...
ngOnInit(): void {
  ...
  //get folders
  this.fileService.getCategoryGroupSub2(this.id, this.departUser).subscribe(
    data => {
      this.category = data;
    },
  ...
  this.fileService.getCategoryFileSub1(this.id, this.departUser).subscribe(
    data => {
      this.files = data;
...
deleteFile(fileid, filename){
  ...
  this.fileService.deleteFileSub1(fileid, filename, this.departUser, this.emailUser).subscribe(
    data => {
      this.notifier.notify('success', 'The file is deleted successfuly!');
      // window.location.href = '/page/' + this.id;
      this.router.routeReuseStrategy.shouldReuseRoute = () => false;
      this.router.onSameUrlNavigation = 'reload';
      this.router.navigate(['/sub1/' + this.id]);
    },
...
pagFiles(){
  ...
  //
  this.departUser = localStorage.getItem('archiving_depart');
  this.fileService.getCategoryFileSub1(catid, this.departUser).subscribe(
    data => {
      this.pagfiles = data;
    },
    error => {
...
addCategiry(angForm2)
{
  ...
  // tslint:disable-next-line:max-line-length
  this.fileService.addCategorySub2(angForm2.value.cname, this.id, depart)
...
// tslint:disable-next-line:max-line-length
this.fileService.addFileSub2(myFormData)
.pipe(first()).subscribe(
data => {
  console.log(data);
...
onRightClick(event: MouseEvent, item) 
  ...
  editCategory(angForm3)
  {
    ...
    {
      // tslint:disable-next-line:max-line-length
      this.fileService.editCategorySub2(this.cname11, angForm3.value.cname2, depart)
      .pipe(first()).subscribe(
      data => {
