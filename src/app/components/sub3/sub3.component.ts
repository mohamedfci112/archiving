...
export class Sub3Component implements OnInit, AfterViewInit {
  ...
  // tslint:disable-next-line:typedef
  assign(depart) {
    this.fileService.assignFileSub3(this.assignId, this.assignedfile, this.departUser, depart).subscribe(
      data => {
  ...
  ngOnInit(): void {
    ...
    //get folders
    this.fileService.getCategoryGroupSub4(this.id, this.departUser).subscribe(
      data => {
        this.category = data;
      },
    ...
    this.fileService.getCategoryFileSub3(this.id, this.departUser).subscribe(
      data => {
        this.files = data;
  ...
  deleteFile(fileid, filename){
    ...
    this.fileService.deleteFileSub3(fileid, filename, this.departUser, this.emailUser).subscribe(
      data => {
        this.notifier.notify('success', 'The file is deleted successfuly!');
        // window.location.href = '/page/' + this.id;
        this.router.routeReuseStrategy.shouldReuseRoute = () => false;
        this.router.onSameUrlNavigation = 'reload';
        this.router.navigate(['/sub3/' + this.id]);
      },
  ...
  pagFiles(){
    ...
    //
    this.departUser = localStorage.getItem('archiving_depart');
    this.fileService.getCategoryFileSub3(catid, this.departUser).subscribe(
      data => {
        this.pagfiles = data;
      },
      error => {
  ...
  addCategiry(angForm2)
  {
    ...
    // tslint:disable-next-line:max-line-length
    this.fileService.addCategorySub4(angForm2.value.cname, this.id, depart)
  ...
  addFile(angForm3)
  {
    ...
    // tslint:disable-next-line:max-line-length
    this.fileService.addFileSub4(myFormData)
    .pipe(first()).subscribe(
    data => {
      console.log(data);
